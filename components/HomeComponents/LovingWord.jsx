  ...
  return (
    <section className="w-full bg-white py-16 px-4 md:px-8">
      <motion.div className="w-full mx-auto text-center" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: false, amount: 0.3 }}>
        {/* Heading */}
        <motion.h2 className="font-laluxes text-[#ff8a80] text-5xl md:text-6xl lg:text-7xl mb-8" variants={blurVariants}>
          Loving Words
        </motion.h2>
        ...
        >
          {testimonials.map((item, index) => (
            <SwiperSlide key={index}>
              <div>
                <p className="text-[#2d5016] text-base md:text-lg lg:text-xl leading-relaxed mb-8">{item.text}</p>
                <p className="font-askha text-[#2d5016] text-sm md:text-base lg:text-lg font-semibold tracking-wide uppercase">{item.author}</p>
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
